...
def render_prediction_results(prediction_data: dict, employee_data: dict):
    ...
    # Jauge de risque
    render_risk_gauge(
        prediction_data.get('attrition_risk', 0),
        prediction_data.get('risk_level', 'Inconnu')
    )
    ...
    with col1:
        render_metric_card(
            "Âge",
            f"{employee_data.get('age', 'N/A')} ans",
            icon="👤"
        )
    ...
    with col4:
        render_metric_card(
            "Revenu mensuel",
            f"{employee_data.get('revenu_mensuel', 'N/A')}€",
            icon="💰"
        )
...
def get_risk_factors_intelligent(employee_data: dict, prediction_data: dict):
    ...
    if satisfaction < 2.0:
        risk_factors.append({
            "icon": "🚨", "title": "Satisfaction très faible",
            "description": f"Satisfaction critique: {satisfaction}/4",
            "weight": 0.25, "category": "satisfaction"
        })
    elif satisfaction < 2.5:
        risk_factors.append({
            "icon": "⚠️", "title": "Satisfaction faible",
            "description": f"Satisfaction moyenne: {satisfaction}/4",
            "weight": 0.15, "category": "satisfaction"
        })
    ...
    if hours > 50:
        risk_factors.append({
            "icon": "⏰", "title": "Surcharge de travail",
            "description": f"{hours}h/semaine (charge excessive)",
            "weight": 0.20, "category": "workload"
        })
    elif hours > 45:
        risk_factors.append({
            "icon": "⚖️", "title": "Charge de travail élevée",
            "description": f"{hours}h/semaine",
            "weight": 0.10, "category": "workload"
        })
    ...
    if years_company < 1:
        risk_factors.append({
            "icon": "🆕", "title": "Nouvel employé",
            "description": f"Moins d'1 an dans l'entreprise (période critique)",
            "weight": 0.18, "category": "tenure"
        })
    elif years_company < 2 and age < 30:
        risk_factors.append({
            "icon": "👶", "title": "Jeune talent",
            "description": f"{age} ans, {years_company} an(s) d'ancienneté (risque de départ)",
            "weight": 0.12, "category": "tenure"
        })
    ...
    if revenu > 0:
        ...
        if revenu < expected_revenu * 0.8:
            risk_factors.append({
                "icon": "💰", "title": "Rémunération sous le marché",
                "description": f"{revenu}€ vs {expected_revenu:.0f}€ attendu",
                "weight": 0.12, "category": "compensation"
            })
    ...
    if burnout >= 3:
        risk_factors.append({
            "icon": "🔥", "title": "Risque de burnout élevé",
            "description": f"Score burnout: {burnout}/4 (niveau critique)",
            "weight": 0.20, "category": "wellbeing"
        })
    elif burnout >= 2:
        risk_factors.append({
            "icon": "😰", "title": "Stress important",
            "description": f"Score burnout: {burnout}/4",
            "weight": 0.10, "category": "wellbeing"
        })
    ...
    if equilibre <= 2 and having_kids == 'Oui':
        risk_factors.append({
            "icon": "👨‍👩‍👧‍👦", "title": "Conflit travail/famille",
            "description": f"Équilibre {equilibre}/4 avec enfants à charge",
            "weight": 0.15, "category": "worklife"
        })
    elif equilibre <= 2:
        risk_factors.append({
            "icon": "⚖️", "title": "Déséquilibre vie pro/perso",
            "description": f"Équilibre {equilibre}/4",
            "weight": 0.08, "category": "worklife"
        })
...
def get_personalized_recommendations(employee_data: dict, prediction_data: dict):
    ...
    if risk_level == 'Très élevé' or risk_percentage > 80:
        ...
        recommendations["urgent"].append("💬 Évaluer immédiatement les sources d'insatisfaction principales")
    ...
    if "satisfaction" in factor_categories:
        ...
        if high_satisfaction_risk:
            recommendations["urgent"].append("🔍 Audit complet de satisfaction avec questionnaire détaillé")
            recommendations["short_term"].append("💰 Évaluer la rémunération par rapport au marché")
            recommendations["short_term"].append("🏡 Discuter des conditions de travail et environnement")

    if "workload" in factor_categories:
        workload_issues = factor_categories["workload"]
        hours = employee_data.get('nombre_heures_travailless', 40)
        ...
        if hours > 50:
            ...
            recommendations["short_term"].append("📊 Analyse et optimisation de la charge de travail")

        recommendations["medium_term"].append("🛠️ Formation sur la gestion du temps et priorisation")

    if "career" in factor_categories:
        career_issues = factor_categories["career"]
        last_promotion = employee_data.get('annees_depuis_la_derniere_promotion', 0)
        ...
        if last_promotion > 3:
            ...
            recommendations["medium_term"].append("🎓 Identifier les compétences à développer pour promotion")
            if niveau < 3:
                recommendations["medium_term"].append("🎯 Préparer un objectif de promotion dans les 6-12 mois")
    ...
    if "compensation" in factor_categories:
        ...
        recommendations["medium_term"].append("🎁 Envisager avantages complémentaires (flexibilité, formation)")
    ...
    if "wellbeing" in factor_categories:
        ...
        recommendations["medium_term"].extend([
            "🏃‍♂️ Promouvoir activités bien-être",
            "📱 Encourager déconnexion hors travail heures"
        ])
...
def main():
    ...
    st.set_page_config(
        page_title="Prédiction d'Attrition",
        page_icon="🎯",
        layout="wide"
    )
            ...
            if st.button("🎯 Prédire l'attrition", type="secondary"):
                try:
                    ...
                    with st.spinner("Analyse avec le modèle de machine learning..."):
                        prediction_data = api_client.predict_attrition(st.session_state.selected_employee)
