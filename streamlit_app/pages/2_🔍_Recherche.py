            ...
            if employee:
                ...
                with tab1:
                    ...
                    with col2:
                        ...
                        st.metric("Distance Domicile", f"{employee.get('distance_domicile_travail', 'N/A')} km")
                ...
                with tab2:
                    ...
                    with col2:
                        st.metric("Revenu Mensuel", f"{employee.get('revenu_mensuel', 0):,.0f} €")
                        st.metric("Heures Supplémentaires", employee.get("heure_supplementaires", "N/A"))
                        st.metric("Niveau Hiérarchique", employee.get("niveau_hierarchique_poste", "N/A"))

                with tab3:
                    col1, col2 = st.columns(2)
                    with col1:
                        st.metric("Années d'Expérience", employee.get("annee_experience_totale", "N/A"))
                        st.metric("Années dans l'Entreprise", employee.get("annees_dans_l_entreprise", "N/A"))
                        st.metric("Années au Poste Actuel", employee.get("annees_dans_le_poste_actuel", "N/A"))
                    with col2:
                        st.metric("Expériences Précédentes", employee.get("nombre_experiences_precedentes", "N/A"))
                        st.metric("Formations Suivies", employee.get("nb_formations_suivies", "N/A"))
                        st.metric("Dernière Promotion", f"{employee.get('annees_depuis_la_derniere_promotion', 'N/A')} ans")

                with tab4:
                    col1, col2 = st.columns(2)
                    with col1:
                        st.metric("Satisfaction Environnement", f"{employee.get('satisfaction_employee_environnement', 'N/A')}/4")
                        st.metric("Satisfaction Travail", f"{employee.get('satisfaction_employee_nature_travail', 'N/A')}/4")
                    with col2:
                        st.metric("Satisfaction Équipe", f"{employee.get('satisfaction_employee_equipe', 'N/A')}/4")
                        st.metric("Équilibre Vie Pro/Perso", f"{employee.get('satisfaction_employee_equilibre_pro_perso', 'N/A')}/4")
                    ...
                    st.metric(
                        ...
                        delta=f"{((satisfaction_moy - 2.5) / 2.5 * 100):.0f}% vs moyenne" if satisfaction_moy else None
                    )
